...
var UserUpdatedTask = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(_) {
  return [new Model(new None(), toList([])), read_localstorage2("task")];
}
...
function update2(model, msg) {
  if (msg instanceof UserUpdatedTask) {
    let input$1 = msg[0];
    return [
      model.withFields({ task: new Some(input$1) }),
      write_localstorage2("task", input$1)
    ];
  } else if (msg instanceof CacheUpdatedMessage && msg[0].isOk()) {
    let task = msg[0][0];
    return [model.withFields({ task: new Some(task) }), none()];
  } else if (msg instanceof CacheUpdatedMessage && !msg[0].isOk()) {
    return [model, none()];
  } else {
    let $ = model.task;
    if ($ instanceof Some) {
      ...
      return [
        new Model(
          new None(),
          concat2(toList([model.tasks, toList([task])]))
        ),
        none()
      ];
    } else {
...
function list_task(task) {
  return li(toList([]), toList([text(task)]));
}
function list_items(tasks) {
  return map2(tasks, list_task);
}
function input2(label2, placeholder2, for$, type_2, value3, msg) {
  return div(
...
function view(model) {
  let task = unwrap(model.task, "");
  return div(
    toList([
      class$("mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-screen py-10")
    ]),
    toList([
      div(
        toList([class$("mx-auto max-w-3xl text-center")]),
        toList([
          h2(
            toList([
              class$(
            ...
            toList([text("Gleam TODO App")])
          ),
          p(
            toList([class$("mt-6 text-lg leading-8 text-gray-600")]),
            toList([
              text(
          ...
          form(
            ...
            toList([
              input2(
                ...
                task,
                (var0) => {
                  return new UserUpdatedTask(var0);
                }
              )
            ])
          ),
          ul(toList([]), list_items(model.tasks))
        ])
      )
    ])
  );
